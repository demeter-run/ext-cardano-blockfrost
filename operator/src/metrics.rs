...
use crate::{get_config, BlockfrostPort, Error, State};
...
impl Metrics {
    ...
    pub fn reconcile_failure(&self, crd: &BlockfrostPort, e: &Error) {
    ...
    pub fn count_dcu_consumed(
        ...
        let service = format!("{}-{}", BlockfrostPort::kind(&()), network);
        let service_type = format!(
            "{}.{}",
            BlockfrostPort::plural(&()),
            BlockfrostPort::group(&())
        );
        let tenancy = "proxy";
...
pub fn run_metrics_collector(state: Arc<State>) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            let query = format!(
                "sum by (consumer, network, resource_name, tier) (increase(blockfrost_proxy_http_total_request{{status_code!~\"401|429|503\"}}[{start}s] @ {}))",
            ...
            for result in response.data.result {
                if result.value == 0.0
                    ...
                    || result.metric.resource_name.is_none()
                ...
                let resource_name = result.metric.resource_name.unwrap();
                ...
                state
                    .metrics
                    .count_dcu_consumed(project, &network, &resource_name, &tier, dcu);
            }
        }
    });
}
